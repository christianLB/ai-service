-- Script to restore missing financial tables after fixing the schema cleanup issue
-- This creates only the tables that are missing (clients, invoices, invoice_sequences)
...
-- 2. Create invoice_sequences table if not exists
...
GRANT ALL ON SEQUENCE financial.invoice_sequences_id_seq TO ai_user;

-- 6. Verify tables were created
SELECT 'Tables created successfully:' as status;
SELECT table_name, 
       CASE 
         WHEN table_name IN ('clients', 'invoices', 'invoice_sequences') THEN '✅ Created'
