#!/bin/bash
# Apply fix for production endpoints

source .make.env

echo "Applying production endpoints fix..."

# Apply the SQL using docker exec
ssh k2600x@192.168.1.11 << 'EOF'
sudo /usr/local/bin/docker exec -i ai-postgres psql -U ai_user -d ai_service << 'EOSQL'
