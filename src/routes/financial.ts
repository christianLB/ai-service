    ...
    try {
      ...
      if (result.success) {
        ...
        res.status(400).json({
          ...result,
          message:
            'Failed to initiate BBVA account setup. Please check your configuration and try again.',
        });
      }
    } catch (error) {
      ...
      res.status(500).json({
        ...
        details: error instanceof Error ? error.message : 'Unknown error',
        message:
          'An unexpected error occurred while setting up your BBVA account. Please try again later.',
      });
    }
  }
);
    ...
    try {
      ...
      if (!uuidRegex.test(requisitionId)) {
        ...
        res.status(400).json({
          ...
          message:
            'The requisitionId must be a valid UUID (e.g., 123e4567-e89b-12d3-a456-426614174000)',
        });
        return;
      }
  ...
  async (req: Request, res: Response, _next: NextFunction) => {
    try {
      ...
      if (!requisitionStatus.success) {
        console.error(
          `[requisition-status] Failed to get requisition status: ${requisitionStatus.error}`
        );
        res.status(404).json({
          success: false,
      ...
      const response = {
        ...
        data: {
          ...
          consentGiven:
            requisitionStatus.data?.status === 'LN' || requisitionStatus.data?.status === 'LINKED',
          ...
          statusDetails: {
            ...
            isLinked:
              requisitionStatus.data?.status === 'LN' ||
              requisitionStatus.data?.status === 'LINKED',
      ...
      res.status(500).json({
        ...
        details: error instanceof Error ? error.message : 'Unknown error',
        message:
          'An unexpected error occurred while checking the requisition status. Please try again later.',
      });
    }
  }
);
  ...
  async (req: Request, res: Response, _next: NextFunction) => {
    try {
      ...
      try {
        ...
        const validationErrors = transactionImportService.validateTransactions(
  ...
  async (req: Request, res: Response, _next: NextFunction) => {
    try {
      ...
      for (const account of accounts.filter(
        (a: Account) => a.type === 'bank_account' && a.accountId
      )) {
      ...
      if (syncStats) {
        transformedStats = {
          ...
          summary: {
            ...
            total_transactions:
              parseInt(syncStats.summary?.total_transactions_synced as string) || 0,
  ...
  async (req: Request, res: Response, _next: NextFunction) => {
    try {
      ...
      try {
        ...
        diagnosis.checks.push({
          ...
          details: {
            ...
            secretIdFormat: secretId
              ? `${secretId.substring(0, 8)}...${secretId.substring(secretId.length - 4)}`
              : 'missing',
            baseUrl: baseUrl,
      ...
      res.json({
        ...
        summary: allPassed
          ? 'All checks passed'
          : 'Some checks failed - review diagnosis for details',
      });
    } catch (error) {
...
function getRecommendations(status: any): string[] {
  ...
  if (!status.configured) {
    recommendations.push(
      'Configure GoCardless credentials using POST /api/financial/gocardless/credentials'
    );
  ...
  if (status.configured && !status.authenticated) {
    recommendations.push(
      'Authentication is failing. Check if credentials are from the correct environment (production vs sandbox)'
    );
  ...
  if (!status.credentials.hasSecretId || !status.credentials.hasSecretKey) {
    recommendations.push(
      'Missing required credentials. Both secret_id and secret_key must be configured'
    );
  }

  if (status.credentials.secretIdLength && status.credentials.secretIdLength !== 36) {
    recommendations.push(
      'Secret ID should be a UUID (36 characters). Current length: ' +
        status.credentials.secretIdLength
    );
  }

  if (status.credentials.secretKeyLength && status.credentials.secretKeyLength !== 43) {
    recommendations.push(
      'Secret Key should be 43 characters. Current length: ' + status.credentials.secretKeyLength
    );
  }
...
router.get(
  ...
  async (req: Request, res: Response, _next: NextFunction) => {
    try {
      ...
      // Debug test
      try {
        const debugResult = await (integrationConfigService as any).debugGetConfig(
          'gocardless',
          'secret_id'
        );
...
router.get(
  ...
  async (req: Request, res: Response, _next: NextFunction) => {
    try {
      ...
      // Calculate changes
      const incomeChange =
        previousReport.summary.totalIncome !== '0'
          ? ((parseFloat(currentReport.summary.totalIncome) -
              parseFloat(previousReport.summary.totalIncome)) /
              parseFloat(previousReport.summary.totalIncome)) *
            100
          : 0;

      const expenseChange =
        previousReport.summary.totalExpenses !== '0'
          ? ((parseFloat(currentReport.summary.totalExpenses) -
              parseFloat(previousReport.summary.totalExpenses)) /
              parseFloat(previousReport.summary.totalExpenses)) *
            100
          : 0;
      ...
      res.json({
        ...
        data: {
          ...
          changes: {
            ...
            net:
              previousReport.summary.netIncome !== '0'
                ? ((parseFloat(currentReport.summary.netIncome) -
                    parseFloat(previousReport.summary.netIncome)) /
                    Math.abs(parseFloat(previousReport.summary.netIncome))) *
                  100
                : 0,
          },
          period: {
...
router.get(
  ...
  async (req: Request, res: Response, _next: NextFunction) => {
    ...
    `;
      ...
      const topCategoriesResult = await databaseService.pool.query(topCategoriesQuery, [
        currentMonthStart,
        currentMonthEnd,
      ]);
