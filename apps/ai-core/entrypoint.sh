...
# ============================================================================
# AI Core Service Entrypoint - F5 Enhanced Version
# ============================================================================
# Features:
...
try {
  ...
  if (env.NODE_ENV === 'production' && !env.OPENAI_API_KEY) {
    console.error('❌ ERROR: OpenAI API key required in production');
    process.exit(1);
  }
...
# ============================================================================
# Step 3: Wait for Redis (Used for Caching)
# ============================================================================
...
while [ $i -lt $RETRIES ]; do
  if nc -z "$REDIS_HOST" "$REDIS_PORT" 2>/dev/null; then
    echo "[$SERVICE_NAME] ✅ Redis is ready (used for AI response caching)"
    break
  fi
  i=$((i+1))
...
echo "[$SERVICE_NAME] AI providers: $(node -e "
const env = process.env;
const providers = [];
if (env.OPENAI_API_KEY) providers.push('OpenAI');
if (env.ANTHROPIC_API_KEY) providers.push('Anthropic');
console.log(providers.length ? providers.join(', ') : 'None');
")"
echo "[$SERVICE_NAME] ============================================"
