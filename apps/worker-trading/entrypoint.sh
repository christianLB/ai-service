...
# ============================================================================
# Trading Worker Service Entrypoint - F5 Enhanced Version
# ============================================================================
# Features:
...
try {
  const env = getEnv('worker-trading');
  console.log('✅ Environment validation successful');
  console.log('  NODE_ENV:', env.NODE_ENV);
  console.log('  PORT:', env.PORT || 3102);
  console.log('  Queue Prefix:', env.QUEUE_PREFIX || 'ai-service');
  console.log('  Remove on Complete:', env.QUEUE_DEFAULT_REMOVE_ON_COMPLETE);
  console.log('  Remove on Fail:', env.QUEUE_DEFAULT_REMOVE_ON_FAIL);
  console.log('  Trading APIs:');
...
# ============================================================================
# Step 3: Wait for Redis (Critical for BullMQ and Market Data Cache)
# ============================================================================
...
while [ $i -lt $RETRIES ]; do
  if nc -z "$REDIS_HOST" "$REDIS_PORT" 2>/dev/null; then
    echo "[$SERVICE_NAME] ✅ Redis is ready (critical for job queue and market data)"
    break
  fi
  i=$((i+1))
...
# ============================================================================
echo "[$SERVICE_NAME] Starting worker on port ${PORT:-3102}..."
echo "[$SERVICE_NAME] Environment: ${NODE_ENV:-development}"
echo "[$SERVICE_NAME] Queue processing: trading-operations"
echo "[$SERVICE_NAME] Job types: market-analysis, order-execution, bot-management"
echo "[$SERVICE_NAME] FSM states: Idle → Live → Trading → Stopped"
echo "[$SERVICE_NAME] ============================================"
