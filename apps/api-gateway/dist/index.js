...
app.get("/api/financial/clients", async (req, res) => {
    ...
    try {
        ...
        if (!result.data) {
            ...
            if (status === 400) {
                res.status(400).json({
                    message: result.error ? result.error?.message || 'Invalid request parameters' : 'Invalid request parameters',
                    code: 'BAD_REQUEST'
                });
            }
            else {
...
app.get("/api/financial/transactions", async (req, res) => {
    try {
        ...
        const keys = ["accountId", "type", "status", "dateFrom", "dateTo", "minAmount", "maxAmount", "search"];
        for (const k of keys) {
            const v = typeof req.query[k] === 'string' ? req.query[k] : undefined;
            if (v)
                params.set(k, v);
        }
...
app.get("/api/financial/transactions/:id", async (req, res) => {
    try {
        ...
        if (!r.ok) {
            ...
            if (status === 400)
                return res.status(400).json({ message: 'Invalid transaction ID', code: 'BAD_REQUEST' });
...
app.get("/api/financial/transactions/export", async (req, res) => {
    try {
        ...
        const keys = ["accountId", "dateFrom", "dateTo"];
        for (const k of keys) {
            const v = typeof req.query[k] === 'string' ? req.query[k] : undefined;
            if (v)
                params.set(k, v);
...
app.get("/api/financial/attachments/:id", async (req, res) => {
    try {
        ...
        if (!r.ok) {
            ...
            if (status === 400)
                return res.status(400).json({ message: 'Invalid attachment ID', code: 'BAD_REQUEST' });
            return res.status(502).json({ message: msg || 'Service unavailable', code: 'GATEWAY_ERROR' });
        }
        const data = await r.json();
        res.json(data);
...
app.put("/api/integrations/configs/:type/:key", async (req, res) => {
    try {
        ...
        const params = [configValue ?? "", description ?? null, type, key];
        let q = `
...
app.post("/api/integrations/test/:type", async (req, res) => {
    try {
        ...
        if (type === "gocardless") {
            ...
            if (!sid || !sk)
                return res.status(400).json({ success: false, data: { integrationType: type, isValid: false, message: "Missing secret_id/secret_key" } });
...
app.post("/api/financial/gocardless/sync/accounts", async (_req, res) => {
    try {
        ...
        const text = await r.text();
        if (!r.ok)
            return res.status(r.status).send(text || 'Sync accounts failed');
        try {
...
app.post("/api/financial/gocardless/sync/transactions", async (req, res) => {
    try {
        ...
        const text = await r.text();
        if (!r.ok)
            return res.status(r.status).send(text || 'Sync transactions failed');
        try {
            return res.json(JSON.parse(text));
        }
        catch {
